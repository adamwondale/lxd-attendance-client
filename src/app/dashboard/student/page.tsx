    ...
    <div className="w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-6 sm:py-8 lg:py-10 space-y-10 sm:space-y-12 text-primary bg-[#F9F9F8] min-h-full font-sans">
      ...
      <section className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        {[
          [
            'Present',
            summaryData?.myAttendanceSummary?.presentDays ?? 0,
            'text-[#0A0A0A]',
          ],
          [
            'Late',
            summaryData?.myAttendanceSummary?.lateDays ?? 0,
            'text-[#0A0A0A]',
          ],
          [
            'Penalties',
            `${summaryData?.myAttendanceSummary?.totalPenalty ?? 0} ETB`,
            'text-[#E54D2E]',
          ],
        ].map(([label, value, colorClass]) => (
          <div
            key={String(label)}
            className="p-4 sm:p-7 lg:p-9 min-h-[150px] sm:min-h-[190px] flex flex-col items-center sm:items-start justify-center text-center sm:text-left border border-[#E5E5E4] bg-surface rounded-2xl interactive hover:-translate-y-0.5"
          >
      ...
      <section className="space-y-4">
        ...
        <div className="bg-surface border border-[#E5E5E4] rounded-xl shadow-sm">
          {summaryLoading && !summaryData?.myAttendanceSummary ? (
                ...
                .map((log: any) => (
                  ...
                  >
                    <div>
                      ...
                      <p
                        className={`text-[12px] sm:text-[13px] mt-0.5 sm:mt-1 ${log.isLate ? 'text-accent' : 'text-muted'}`}
                      >
                        {log.isLate
                    ...
                    <div className="text-right">
                      {log.isLate ? (
                        <>
                          <p className="font-serif text-base sm:text-lg text-accent whitespace-nowrap">
