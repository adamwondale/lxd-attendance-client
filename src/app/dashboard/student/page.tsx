  ...
  return (
    <div className="p-6 max-w-[640px] mx-auto space-y-12 text-secondary bg-background min-h-screen font-sans">
      ...
      <section className="flex flex-col gap-4">
        {[
          ['Present', summaryData?.myAttendanceSummary?.presentDays ?? 0, 'text-secondary'],
          ['Late', summaryData?.myAttendanceSummary?.lateDays ?? 0, 'text-secondary'],
          ['Penalties', `${summaryData?.myAttendanceSummary?.totalPenalty ?? 0} ETB`, 'text-primary'],
        ].map(([label, value, colorClass]) => (
          <div key={String(label)} className="p-8 sm:p-10 flex flex-col items-center sm:items-start justify-center text-center sm:text-left border border-border bg-surface">
            <p className="text-[11px] sm:text-[12px] uppercase tracking-widest text-muted font-mono mb-2 sm:mb-4">{label}</p>
      ...
      <section className="space-y-4">
        ...
        <div className="bg-surface border border-border rounded-none shadow-sm">
          {summaryLoading && !summaryData?.myAttendanceSummary ? (
            ...
            <div className="divide-y divide-border">
              {summaryData.myAttendanceSummary.recentLogs.slice(0, 15).map((log: any) => (
                <div key={log.id} className="p-4 sm:p-5 flex items-center justify-between gap-2 sm:gap-4 hover:bg-background transition-colors">
                  <div>
                    <p className="font-medium text-[14px] sm:text-[15px]">{new Date(log.scannedAt).toLocaleDateString()}</p>
                    <p className={`text-[12px] sm:text-[13px] mt-0.5 sm:mt-1 ${log.isLate ? 'text-primary' : 'text-muted'}`}>
                  ...
                  <div className="text-right">
                    {log.isLate ? (
                      <>
                        <p className="font-serif text-base sm:text-lg text-primary whitespace-nowrap">{log.penalty?.amount ?? log.calculatedPenalty ?? 0} ETB</p>
                        <p className="text-[9px] sm:text-[11px] uppercase tracking-widest font-mono text-muted">{log.penalty?.status ?? 'UNPAID'}</p>
                      </>
                    ) : (
          ...
          ) : <div className="p-10 text-center text-[13px] text-muted font-mono uppercase tracking-widest">No attendance records yet.</div>}
        </div>
      </section>
