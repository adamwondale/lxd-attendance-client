...
import { Loader2, Edit2, Trash2, X, Plus, Search, ChevronLeft, ChevronRight, Eye, EyeOff, AlertTriangle } from "lucide-react"
...
const LIST_STUDENTS = gql`
  query ListStudents {
    listStudents {
      ...
      memberships { cohortId sessionId status cohort { name } session { name } }
    }
  }
`
...
const UPDATE_STUDENT = gql`
  mutation AdminUpdateStudent($id:String!,$name:String,$email:String,$username:String) {
    adminUpdateStudent(id:$id,name:$name,email:$email,username:$username) { id name email username }
  }
`
const DELETE_STUDENT = gql`mutation AdminDeleteStudent($id:String!) { adminDeleteStudent(id:$id) }`
const ENROLL_STUDENT = gql`mutation AdminEnrollStudent($userId:String!,$cohortId:String!,$sessionId:String!) { adminEnrollStudent(userId:$userId,cohortId:$cohortId,sessionId:$sessionId) }`
const UPDATE_MEMBERSHIP = gql`mutation AdminUpdateStudentMembership($userId:String!,$cohortId:String!,$sessionId:String!) { adminUpdateStudentMembership(userId:$userId,cohortId:$cohortId,sessionId:$sessionId) }`
const REMOVE_FROM_COHORT = gql`mutation AdminRemoveStudentFromCohort($userId:String!,$cohortId:String!) { adminRemoveStudentFromCohort(userId:$userId,cohortId:$cohortId) }`
const ON_STUDENTS_UPDATED = gql`subscription OnStudentsUpdated { onStudentsUpdated }`

const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
const PHONE_RE = /^[0-9+\-\s()]{7,15}$/
const BLANK_CREATE = { name: "", email: "", phone: "", username: "", password: "", cohortId: "", sessionId: "" }
...
function validateCreateForm(f: typeof BLANK_CREATE) {
  ...
  if (!PHONE_RE.test(f.phone)) e.phone = "Enter a valid phone number (7-15 digits)."
  if (!f.username.trim()) e.username = "Username is required."
  if (f.password.length < 6) e.password = "At least 6 characters."
  return e
}

function validateEditForm(f: { name: string; email: string; username: string }) {
  const e: Partial<typeof f> = {}
  if (!f.name.trim()) e.name = "Full name is required."
  if (!EMAIL_RE.test(f.email)) e.email = "Enter a valid email address."
  if (!f.username.trim()) e.username = "Username is required."
...
function Field({ label, id, type = "text", placeholder, value, onChange, onBlur, error, disabled, hint }: {
...
function SelectField({ label, id, value, onChange, disabled, children }: {
  label: string; id: string; value: string; onChange: (v: string) => void; disabled?: boolean; children: React.ReactNode
}) {
  return (
    <div className="flex flex-col gap-1">
      <label htmlFor={id} className="font-mono text-[11px] uppercase tracking-widest text-muted">{label}</label>
...
function ConfirmDeleteModal({ name, onConfirm, onCancel, loading }: { name: string; onConfirm: () => void; onCancel: () => void; loading: boolean }) {
  return (
    <AlertModal
      isOpen={true}
      onClose={onCancel}
...
function CreateStudentModal({ cohortData, onClose, onSuccess }: { cohortData: any; onClose: () => void; onSuccess: () => void }) {
  ...
  const [touched, setTouched] = useState<Partial<Record<keyof typeof BLANK_CREATE, boolean>>>({})

  const set = (k: keyof typeof BLANK_CREATE) => (v: string) => setForm(f => ({ ...f, [k]: v }))
  const touch = (k: keyof typeof BLANK_CREATE) => () => setTouched(t => ({ ...t, [k]: true }))
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault()
    const allTouched = Object.fromEntries(Object.keys(BLANK_CREATE).map(k => [k, true])) as any
    setTouched(allTouched)
    const errs = validateCreateForm(form)
    setErrors(errs)
    if (Object.keys(errs).length) return
    try {
      await createStudent({ variables: { ...form, cohortId: form.cohortId || undefined, sessionId: form.sessionId || undefined } })
      toast.success("Student registered successfully.")
      onSuccess()
    } catch (err: any) { toast.error(err.message || "Registration failed.") }
  }

  const sessions = cohortData?.listCohorts?.find((c: any) => c.id === form.cohortId)?.sessions || []

  return (
    <Modal isOpen={true} onClose={onClose}>
      <ModalHeader title="Register Student" subtitle="Students" onClose={onClose} />
      <ModalBody>
        <form id="create-student-form" onSubmit={submit} noValidate className="space-y-8">
          <div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <Field id="c-phone" label="Phone Number" type="tel" placeholder="+251 9xx xxx xxxx" value={form.phone} onChange={set("phone")} onBlur={touch("phone")} error={errors.phone} hint="International format preferred" />
              <Field id="c-username" label="Username" placeholder="abebe2024" value={form.username} onChange={set("username")} onBlur={touch("username")} error={errors.username} />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <SelectField id="c-session" label="Session" value={form.sessionId} onChange={set("sessionId")} disabled={!form.cohortId}>
...
function EditStudentModal({ student, cohortData, onClose, onRefetch }: { student: any; cohortData: any; onClose: () => void; onRefetch: () => void }) {
  ...
  const [form, setForm] = useState({ name: student.name, email: student.email, username: student.username || "" })
  ...
  const [localMemberships, setLocalMemberships] = useState<any[]>(student.memberships || [])
  ...
  const handleEnroll = async () => {
    ...
    try {
      await enrollStudent({ variables: { userId: student.id, cohortId: selectedCohort, sessionId: selectedSession } })
      const cohort = cohortData?.listCohorts?.find((c: any) => c.id === selectedCohort)
      const sess = cohort?.sessions?.find((s: any) => s.id === selectedSession)
      setLocalMemberships(m => [...m, { cohortId: selectedCohort, sessionId: selectedSession, cohort: { name: cohort?.name }, session: { name: sess?.name } }])
      toast.success("Enrolled in cohort.")
      setNewEnrollment(false); setSelectedCohort(""); setSelectedSession("")
      onRefetch()
  ...
  const handleUpdateSession = async (cohortId: string, newSessionId: string) => {
    try {
      await updateMembership({ variables: { userId: student.id, cohortId, sessionId: newSessionId } })
      toast.success("Session updated."); onRefetch()
  ...
  const handleRemoveCohort = async (cohortId: string) => {
    try {
      ...
      toast.success("Removed from cohort."); onRefetch()
    } catch (err: any) { toast.error(err.message || "Failed to remove.") }
  }

  const enrollSessions = cohortData?.listCohorts?.find((c: any) => c.id === selectedCohort)?.sessions || []
        ...
        <form id="edit-profile-form" onSubmit={handleUpdate} noValidate className="p-6 border-b border-border">
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-4">
            ...
            <div className="sm:col-span-2">
              <Field id="e-username" label="Username" value={form.username} onChange={v => setForm(f => ({ ...f, username: v }))} onBlur={() => setTouched(t => ({ ...t, username: true }))} error={errors.username} />
            </div>
          </div>
            ...
            {newEnrollment && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                <div className="bg-background border border-border p-4 mb-4 grid sm:grid-cols-2 gap-3">
                  ...
                  <SelectField id="enroll-session" label="Session" value={selectedSession} onChange={setSelectedSession} disabled={!selectedCohort}>
                    <option value="">Select session...</option>
...
export default function StudentsPage() {
  ...
  const filteredStudents = (data?.listStudents || []).filter((s: any) =>
    `${s.name} ${s.email} ${s.username || ""}`.toLowerCase().includes(search.toLowerCase())
  )
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success("Student removed."); setConfirmDelete(null); refetch()
    } catch (err: any) { toast.error(err.message || "Failed to remove student.") }
  }
  ...
  return (
    <div className="p-6 sm:p-10 space-y-8 min-h-screen bg-background">
      ...
      <AnimatePresence>
        {showCreate && <CreateStudentModal key="create" cohortData={cohortData} onClose={() => setShowCreate(false)} onSuccess={() => { setShowCreate(false); refetch() }} />}
        {editingStudent && <EditStudentModal key="edit" student={editingStudent} cohortData={cohortData} onClose={() => setEditingStudent(null)} onRefetch={refetch} />}
        {confirmDelete && <ConfirmDeleteModal key="confirm" name={confirmDelete.name} loading={deleting} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} />}
      </AnimatePresence>
    </div>
  )
}