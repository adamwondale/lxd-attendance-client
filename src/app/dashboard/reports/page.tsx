...
import { Printer, FileSpreadsheet, FileText, RefreshCw, ChevronLeft, ChevronRight } from "lucide-react"
...
type ReportCohort = { id: string; name: string; sessions: Array<{ id: string; name: string }> }
type ReportCohortsData = { listCohorts: ReportCohort[] }
type ReportRow = { id: string; date: string; status: string; traineeId: string; traineeName: string; cohortName: string; sessionName: string; latenessMinutes: number; penalty: number }
...
export default function ReportsPage() {
  ...
  const { data: cohortData } = useQuery<ReportCohortsData>(COHORTS, { fetchPolicy: "cache-first" })
  const { data, loading, refetch } = useQuery<ReportData>(REPORT, { variables: { startDate, endDate, cohortId: cohortId || undefined, sessionId: sessionId || undefined }, fetchPolicy: "cache-and-network" })
  ...
  const setPreset = (value: string) => { setPeriod(value); setPage(1); const r = rangeFor(value); setStartDate(r.start); setEndDate(r.end) }
  const exportExcel = () => {
    ...
    const body = rows.map((r: any) => `<tr><td>${escape(r.date)}</td><td>${escape(r.traineeName)}</td><td>${escape(r.cohortName)}</td><td>${escape(r.sessionName)}</td><td>${escape(r.status)}</td><td>${r.latenessMinutes || 0}</td><td>${r.penalty || 0}</td></tr>`).join("")
    ...
    <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-10 py-6 sm:py-8 lg:py-10 space-y-8 text-[#0A0A0A]">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
        ...
        <div className="flex gap-3">
          <button onClick={() => window.print()} className="h-11 px-5 border border-[#E5E5E4] bg-white text-[#0A0A0A] hover:bg-[#F9F9F8] transition-colors rounded-xl flex items-center gap-2 font-mono text-[11px] uppercase tracking-widest">
            <Printer className="w-4 h-4"/> Print
          </button>
          <div className="print:hidden flex flex-wrap gap-2">
            <button onClick={exportExcel} className="h-11 px-5 bg-[#0A0A0A] text-white hover:bg-[#1C1C1C] transition-all rounded-xl flex items-center gap-2 font-mono text-[11px] uppercase tracking-widest">
              <FileSpreadsheet className="w-4 h-4"/> Export Excel
            </button>
            <button onClick={exportPDF} className="h-11 px-5 border border-[#E5E5E4] bg-white hover:bg-[#F9F9F8] transition-all rounded-xl flex items-center gap-2 font-mono text-[11px] uppercase tracking-widest">
      ...
      <div className="print:hidden bg-[#FFFFFF] border border-[#E5E5E4] p-6 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
        <select value={period} onChange={e => setPreset(e.target.value)} className="h-11 border border-[#E5E5E4] bg-[#F9F9F8] px-3 text-[14px] font-sans focus:border-[#0A0A0A] outline-none transition-colors rounded-xl appearance-none cursor-pointer">
          <option value="week">Last 7 days</option>
          <option value="month">This month</option>
          <option value="year">This year</option>
          <option value="custom">Custom</option>
        </select>
        <input type="date" value={startDate} onChange={e => {setPeriod("custom"); setStartDate(e.target.value)}} className="h-11 border border-[#E5E5E4] bg-[#F9F9F8] px-3 text-[14px] font-sans focus:border-[#0A0A0A] outline-none transition-colors rounded-xl" />
        <input type="date" value={endDate} onChange={e => {setPeriod("custom"); setEndDate(e.target.value)}} className="h-11 border border-[#E5E5E4] bg-[#F9F9F8] px-3 text-[14px] font-sans focus:border-[#0A0A0A] outline-none transition-colors rounded-xl" />
        <select value={cohortId} onChange={e => {setPage(1); setCohortId(e.target.value); setSessionId("")}} className="h-11 border border-[#E5E5E4] bg-[#F9F9F8] px-3 text-[14px] font-sans focus:border-[#0A0A0A] outline-none transition-colors rounded-xl appearance-none cursor-pointer">
          <option value="">All Cohorts</option>
          {cohorts.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}
        </select>
        <div className="flex gap-2">
          <select value={sessionId} onChange={e => {setPage(1); setSessionId(e.target.value)}} className="h-11 border border-[#E5E5E4] bg-[#F9F9F8] px-3 text-[14px] font-sans focus:border-[#0A0A0A] outline-none transition-colors rounded-xl appearance-none cursor-pointer flex-1">
            <option value="">All Sessions</option>
            {(selectedCohort?.sessions || cohorts.flatMap((c:any) => c.sessions || [])).map((s: any) => <option key={s.id} value={s.id}>{s.name}</option>)}
          </select>
          <button onClick={() => refetch()} className="h-11 w-11 border border-[#E5E5E4] bg-white hover:bg-[#F9F9F8] hover:text-[#0A0A0A] text-[#878786] transition-colors rounded-xl flex items-center justify-center shrink-0">
      ...
      <div className="bg-[#FFFFFF] border border-[#E5E5E4] rounded-xl overflow-hidden">
          ...
          <table className="w-full text-left text-[14px] font-sans">
            ...
            <tbody>
              {loading ? (
                ...
                (printing ? rows : pagedRows).map((r:any) => (
      ...
      {rows.length > PAGE_SIZE && (
        <div className="print:hidden flex flex-col sm:flex-row items-center justify-between gap-3 rounded-2xl border border-[#E5E5E4] bg-white px-4 py-3">
          <span className="font-mono text-[10px] uppercase tracking-widest text-[#878786]">Page {safePage} of {totalPages} · {rows.length} records</span>
